...
class Front {
    ...
    constructor () {
        ...
        this.on('click', '[data-action="product"]', this.onProduct.bind(this));
    ...
    onProduct (event) {
        event.preventDefault();
        this.trigger('action:product', {product: this.getActionTarget(event)});
    }
    ...
    onOrder (event) {
        event.preventDefault();
        this.trigger('action:order', {order: this.getActionTarget(event)});
    }
}